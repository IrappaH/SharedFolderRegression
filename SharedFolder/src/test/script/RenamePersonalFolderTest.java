...
import pages.PersonalFolderRename;
import tests.Autoutil;
import tests.InitialTestSetup;

public class RenamePersonalFolderTest extends InitialTestSetup implements Autoutil {
	private PersonalFolder personalFolder = null;
	private PersonalFolderCreation personalFolderCreation = null;
	private PersonalFolderRename personalFolderRename = null;
	...
	public void verifyFolderIsRenameWithEnglishSinlgeByte() throws Exception {
		...
		personalFolderRename = personalFolder.gotoPersonalFolderRename();
		personalFolderRename.renameFolder(value);
		assertEquals(personalFolder.firstRowName().getText(), value);
	}
	...
	// English 85chars
		...
		public void verifyFolderIsRenameWithEnglishSinlgeByte85chars() throws Exception {
			...
			personalFolderRename = personalFolder.gotoPersonalFolderRename();
			String ActualMsg = personalFolderRename.renameFolderpathIsTooLong(value);
			String ExpectedMsg = getLocalizedMessage("renameMorethan84").trim();			
			assertEquals(ActualMsg, ExpectedMsg);
			personalFolderRename.ErrorOKBtn();
		}
		...
		// unsupport Specialchars
				...
				public void verifyFolderIsRenameWithForwardSlash() throws Exception {
					...
					personalFolderRename = personalFolder.gotoPersonalFolderRename();
					String ActualMsg = personalFolderRename.renameFolderInvalid(value);
	...
	public void verifyFileIsRenamebutton() throws Exception {
		...
		//String value = Autoutil.FileUnsupport[0];
		/*
		 * String projectPath = System.getProperty("user.dir"); Thread.sleep(3000);
		 * personalFolder.selectAllCheckBox().click(); Thread.sleep(1000);
		 * personalFolder.deleteBtn().click(); Thread.sleep(1000);
		 * personalFolder.deleteMsgOkBtn().click(); Thread.sleep(3000);
		 * personalFolder.uploadBtn().click(); Thread.sleep(1000);
		 * personalFolder.uploadFileWithRobot(projectPath + "\\PNGFiles\\");
		 * personalFolder.uploadFileWithRobot("Single.pdf"); Thread.sleep(7000);
		 */
		personalFolderRename = personalFolder.gotoPersonalFileRename();		
		Thread.sleep(1000);
		assertTrue(personalFolderRename.IsDisplayedrenameOption());
