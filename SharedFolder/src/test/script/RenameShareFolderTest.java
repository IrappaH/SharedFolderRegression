...
import pages.SharedFolderRename;
import tests.Autoutil;
import tests.InitialTestSetup;

public class RenameShareFolderTest extends InitialTestSetup implements Autoutil {

	private SharedFolder SharedFolder = null;
	private SharedFolderCreation SharedFolderCreation = null;
	private SharedFolderRename SharedFolderRename = null;
	...
	public void verifyFolderIsRenameWithEnglishSinlgeByte() throws Exception {
		...
		SharedFolderRename = SharedFolder.gotoSharedFolderRename();

		SharedFolderRename.renameFolder(value);
		assertEquals(SharedFolder.firstRowName().getText(), value);
	}
	...
	public void verifyFolderIsRenameWithEnglishSinlgeByte85chars() throws Exception {
		...
		SharedFolderRename = SharedFolder.gotoSharedFolderRename();
		String ActualMsg = SharedFolderRename.renameFolderpathIsTooLong(value);
		String ExpectedMsg = getLocalizedMessage("renameMorethan84").trim();
		assertEquals(ActualMsg, ExpectedMsg);
		SharedFolderRename.ErrorOKBtn();
	}
	...
	public void verifyFolderIsRenameWithForwardSlash() throws Exception {
		...
		SharedFolderRename = SharedFolder.gotoSharedFolderRename();
		String ActualMsg = SharedFolderRename.renameFolderInvalid(value);
	...
	public void verifyFileIsRenamebutton() throws Exception {
		setTestDesc("Verify that Admin can check uploaded file has Rename option in file context menu");

		SharedFolderRename = SharedFolder.gotoSharedFileRename();
		Thread.sleep(1000);
		assertTrue(SharedFolderRename.IsDisplayedrenameOption());
